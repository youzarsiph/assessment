...
export async function createTable() {
  return new Promise((resolve, reject) => {
    db.transaction(
      (tx) => {
        tx.executeSql(
          "create table if not exists menuitems (id integer auto_increment primary key not null, uuid text, title text, price text, category text);"
        );
      },
      reject,
      resolve
...
export async function filterByQueryAndCategories(query, activeCategories) {
  return new Promise((resolve, reject) => {
    db.transaction((tx) => {
      tx.executeSql(
        `select title from menuitems where title like '%${query}%' and category in (${activeCategories});`,
        [],
        (_, { rows }) => {
          resolve(rows._array);
        }
